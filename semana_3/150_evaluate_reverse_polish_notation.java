 ...
 * Visual Explanation:
 * https://neetcode.io/practice/evaluate-reverse-polish-notation
 * Video Solution: https://www.youtube.com/watch?v=iu0082c4HDE
 * 
 * Problem Description:
 * You are given an array of strings tokens that represents an arithmetic
 * expression
 * in Reverse Polish Notation. Evaluate the expression and return an integer
 * that represents
    ...
    public int evalRPN(String[] tokens) {
        ...
        for (String token : tokens) {
            if (token.equals("+") || token.equals("-") || token.equals("*") || token.equals("/")) {
                ...
                switch (token) { // switch no lleva :
                    case "+": // loas cases llevan :
                        stack.push(b + a);
                        break;
                    case "-":
                        stack.push(b - a);

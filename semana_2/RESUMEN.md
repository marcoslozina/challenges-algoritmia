...
1. **Uso de Genéricos Incompatibles con Primitivos**:
   - ❌ `Map<int, char>` (No compila).
   - ✅ `Map<Integer, Character>` (Siempre debes usar Clases "Wrapper" en Java Collections).

2. **Acceso a un `Map` por un `get` nulo (NullPointerException)**:
   - ❌ Incrementar una frecuencia sin revisar si la clave existe: `int value = map.get(k) + 1;` (Lanza error si no existe).
   - ✅ Usar métodos seguros: `map.getOrDefault(k, 0) + 1;`

3. **Ciclos Infinitos en Algoritmos de Detección**:
   - ❌ (En Happy Number) calcular infinitamente sumas sin guardar estados pasados.
   - ✅ Si aplicas el algoritmo y llegas a un valor recurrente antes que el 1, ¡es un ciclo! Para eso sirve agregar e inmediatamente validar con `HashSet`.

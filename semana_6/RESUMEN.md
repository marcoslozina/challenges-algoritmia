...
1. **Variables Sin Reciclar (Espacio innecesario `O(N)`)**:
   - ❌ Es un error normal plantear Programación Dinámica (DP) con un vector de memoria gigante cuando solo dependes del "pasado inmediato" o el "penúltimo pasado".
...
2. **Error al Inicializar la Estructura (Casos Base Iniciales `dp[0], dp[1]`)**:
   - ❌ Omitir rellenar los datos de inicio (casos base de Programación Dinámica - DP). Si `n=1`, fallaría un `dp[1] = ...` si creaste un array `dp = new int[n];` y este por defecto es 0 o dará Index Out of Bounds si pones `[0]` sin validar.
...
3. **Valores Centinela y Max/Mins Falsos**:
   ...
   - ✅ Rellena tu array de Programación Dinámica (DP) con el `Integer.MAX_VALUE`, pre-calculado para dar infinito, usando el `Arrays.fill(dp, Integer.MAX_VALUE)` menos el `dp[0]` inicial en el que tú no usas monedas. Al usar centinelas o IntegerMax prevén el Overflow si le sumas +1 al centinela.
